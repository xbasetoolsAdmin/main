...
$query = mysqli_query($dbcon, "SELECT DISTINCT(`country`) FROM `accounts` WHERE `sold` = '0' ORDER BY country ASC");
	...
	echo '<option value="All'.$row['country'].'"'.$row['country'].'country'; 	} ?&gt;
</select>
							</td>
							<td>
<input class="filterinput form-control input-sm" name="account_sitename" size="3">
	</td>
<td>
...
$query = mysqli_query($dbcon, "SELECT DISTINCT(`resseller`) FROM `accounts` WHERE `sold` = '0' ORDER BY resseller ASC");
	...
	echo '<option value="'.$SellerNick.'"'.$SellerNick.''; 	} ?&gt;
							...
							<td>
								<button id="filterbutton" class="btn btn-primary btn-sm" disabled="">Filter
